/**
 * Catalyst Fieldset — adapted for Gruvbox Dark Hard.
 * Provides Field, Label, Description, FieldGroup, Fieldset.
 */
'use client'
...
export function Legend({ className, ...props }: { className?: string } & Omit<Headless.LegendProps, 'as' | 'className'>) {
  return (
    <Headless.Legend
      data-slot="legend"
      {...props}
      className={clsx(className, 'text-base/6 font-semibold text-gb-fg data-disabled:opacity-50 sm:text-sm/6')}
    />
  )
}
...
export function Label({ className, ...props }: { className?: string } & Omit<Headless.LabelProps, 'as' | 'className'>) {
  return (
    <Headless.Label
      ...
      className={clsx(
        className,
        'block text-[11px] font-semibold uppercase tracking-wider text-gb-faint data-disabled:opacity-50',
      )}
    />
  )
}
...
export function Description({ className, ...props }: { className?: string } & Omit<Headless.DescriptionProps, 'as' | 'className'>) {
  return (
    <Headless.Description
      ...
      {...props}
      className={clsx(className, 'text-xs text-gb-dark data-disabled:opacity-50')}
    />
  )
}
...
export function ErrorMessage({ className, ...props }: { className?: string } & Omit<Headless.DescriptionProps, 'as' | 'className'>) {
  return (
    <Headless.Description
      data-slot="error"
      {...props}
      className={clsx(className, 'text-xs text-gb-red data-disabled:opacity-50')}
    />
  )
}
